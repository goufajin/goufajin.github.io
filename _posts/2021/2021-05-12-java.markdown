...
##### 这类异常实践中直接在方法或构造器中直接抛出，或者传播到自己继承的RuntimeException子类处理（更加具有面向对象编程的思想，异常类见名知义），如下代码片段所示。
```
...
#### @Transactional 事务注解默认对检查型异常不做回滚处理，可以指定rollbackOn属性对需要回滚的异常做定制，如果都要捕获，建议写成Exception，这样检查和非检查异常都实现回滚，如： @Transactional(rollbackFor = Exception.class) 非检查型异常如：数组越界异常等 